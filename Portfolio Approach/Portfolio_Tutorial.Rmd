...
This tutorial illustrates how to calculate systemic risk charges using the portfolio approach. The examples illustrate the principles underlying three different methodologies:
...
The current `R` session needs to call the packages. The `library()` command achieves this:
...
The package `GCPM` requires assigning business lines and countries to each firm. This information is useful when using the *CreditRisk+* option as the loss distribution model. In this tutorial, we use the one-factor model which is an extension of Merton capital structure model of the firm  (Merton, 1974), is the underlying model behind Basel II (Gordy, 2003), and serves as the main model behind the *CreditMetrics* methodology. You may wonder why we do not use *CreditRisk+*. The reason is simple: it is not very precise when PDs are high. What are high PDs? Above 1 percent on annual basis for a *AAA* firm is considered high !

In the one factor model, a single common factor drives the value of the firm and affects its probability of default. The business sector `A` represents this factor, and we assign this to the variable `Business`:

```{r, message=FALSE, warning=FALSE, class.output='bg-danger', class.source='bg-success'}
Business = firmdata$Business
```

Because of the program idiosyncratic data input requirements, which need to be compatible with both the *CreditRisk+* and *CreditMetrics* methods, it is necessary to specify at least two business sectors, even if the second sector is not used at all (keep in mind, however, that in a multifactor model, we will map each business line to a sector):
...
As indicated earlier, the value of the firms, or strictly speaking, their asset value, depends on the value of the common factor. This dependence is captured by the weight matrix, `W`. We specify that the weight corresponding to the factor is `0.3`, where the factor corresponds to the business line `A`. We were forced to include a second business line to comply with the standard data input requirement but since this factor is unnecessary we specify a weight of  `0`: 
